...
check.command.line.arguments <- function (in.opt) {
    ...
    if ( is.null(in.opt$progress)) {
        ...
        if ( ! isatty(stdout()) ) {
            cat("*** WARNING: stdout appears to not be a terminal device. Not using progress bar even though you requested one\n")
            in.opt$progress=FALSE
        }
    }
...
if (opt$threads > 1 ) {
    ...
    for ( kk in 1:mr.data.dims[3]) {
        if (opt$progress) {
            ...
            cat(sprintf("*** Processing Z slice %02d of %02d started at %s\n", kk, mr.data.dims[3], date()))
        }
        statistics.array[ , , kk, ] =
